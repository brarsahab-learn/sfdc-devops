...
class GitHelper {
    ...
    async storySquashRef(storyId, base) {
        ...
        const filesAtMergeBase = new Set((await this.git(["ls-tree", "-r", "--name-only", mergeBase, "--", sourceRoot])).split("\n").filter(Boolean));
        const changedFiles = baseChanges.filter(f => !f.status.startsWith("D") || filesAtMergeBase.has(f.path));
    ...
    async syncWithDev() {
        ...
        if (remoteExists) {
            const behindOrigin = parseInt(await this.git(["rev-list", "--count", `${branch}..origin/${branch}`]).catch(() => "0"), 10) || 0;
            if (behindOrigin > 0) {
                try {
                    ...
                    throw new Error(`Could not sync with origin/${branch} — conflicts exist. Resolve manually:\n` +
    ...
    async commitsBehindOrigin(branch) {
        try {
