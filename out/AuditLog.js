...
exports.METADATA_TYPE_MAP = exports.OPERATION_LABELS = void 0;
exports.metadataFolder = metadataFolder;
exports.metadataTypeForPath = metadataTypeForPath;
exports.buildPackageXml = buildPackageXml;
exports.buildPackageXmlWithGroups = buildPackageXmlWithGroups;
exports.renderAuditHtml = renderAuditHtml;
exports.OPERATION_LABELS = {
...
function metadataFolder(filePath) {
...
function metadataTypeForPath(filePath) {
    const folder = metadataFolder(filePath);
    return folder ? (exports.METADATA_TYPE_MAP[folder] ?? null) : null;
}
function memberNameFromPath(filePath, folder) {
    const parts = filePath.split("/");
 ...
 */
function buildPackageXml(changedFiles) {
    const byType = new Map();
    const unmapped = [];
    for (const f of changedFiles) {
        if (f.change === "deleted") {
            continue;
        } // real deletions belong in destructiveChanges.xml, not here
        const folder = metadataFolder(f.path);
        const type = folder ? exports.METADATA_TYPE_MAP[folder] : undefined;
        if (!type) {
 ...
 */
function buildPackageXmlWithGroups(files) {
...
function searchBlob(entry) {
    ...
    return [
        entry.timestamp, exports.OPERATION_LABELS[entry.operation], entry.storyId, entry.branch, entry.targetEnv,
...
function renderEntry(entry) {
    ...
    const rows = [];
    if (entry.storyId) {
        rows.push(row("Story", entry.storyId));
    }
    if (entry.branch) {
        rows.push(row("Branch", entry.branch));
    }
    if (entry.targetEnv) {
        rows.push(row("Target env", entry.targetEnv));
    }
    if (d.commitMessage) {
        rows.push(row("Commit message", d.commitMessage));
    }
    if (d.version) {
        rows.push(row("Version", d.version));
    }
    if (d.tag) {
        rows.push(row("Tag", d.tag));
    }
    if (d.prUrl) {
        rows.push(row("PR", `<a href="${escapeHtml(d.prUrl)}">${escapeHtml(d.prUrl)}</a>`));
    }
    if (d.releaseNotesPath) {
        rows.push(row("Release notes", d.releaseNotesPath));
    }
    if (d.error) {
        rows.push(row("Error", `<span class="err">${escapeHtml(d.error)}</span>`));
    }
    if (d.conflicts?.length) {
        rows.push(row("Conflicts", d.conflicts.map(escapeHtml).join(", ")));
    }
    if (d.orgConflicts?.length) {
        rows.push(row("Org conflicts (overwritten)", d.orgConflicts.map(c => escapeHtml(`${c.type ? `${c.type} ` : ""}${c.fullName}`)).join(", ")));
    }
    if (d.deployId) {
        rows.push(row("Deploy ID", d.deployId));
    }
    if (d.note) {
        rows.push(row("Note", escapeHtml(d.note)));
    }
    if (d.selectionMode) {
        rows.push(row("Selection", d.selectionMode));
    }
    ...
    const filesBlock = d.changedFiles?.length
        ? `<div class="section-title">Changed files</div><ul class="files">${d.changedFiles.map(f => `<li><span class="change ${f.change}">${f.change}</span> ${escapeHtml(f.path)}</li>`).join("")}</ul>`
        : "";
    const manifestBlock = d.packageXml
        ? `<div class="section-title">package.xml</div><pre class="manifest">${escapeHtml(d.packageXml)}</pre>${d.unmappedFiles?.length
...
<details class="entry outcome-${entry.outcome}"
  ...
  <summary>
    ...
    <span class="op">${escapeHtml(exports.OPERATION_LABELS[entry.operation])}</span>
...
const OUTCOME_ORDER = ["success", "conflict", "failure"];
...
function renderAuditHtml(entries) {
    ...
    const opOptions = Object.entries(exports.OPERATION_LABELS)
