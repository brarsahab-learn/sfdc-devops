...
exports.AuditTrailPanel = void 0;
...
class AuditTrailPanel {
    static createOrShow(gitHelper) {
        if (AuditTrailPanel.current) {
            AuditTrailPanel.current._panel.reveal(vscode.ViewColumn.One);
            AuditTrailPanel.current._refresh();
            return;
        }
        const panel = vscode.window.createWebviewPanel("sfDevopsAuditTrail", "Salesforce-DevOps Audit Trail", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    constructor(panel, _gitHelper) {
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (msg.command === "export") {
                await this._handleExport(msg);
                return;
            }
            if (msg.command === "trim") {
                await this._handleTrim(msg.days);
                return;
            }
    ...
    async _handleExport(msg) {
        try {
            ...
            if (msg.filterOp) {
                entries = entries.filter(e => e.operation === msg.filterOp);
            }
            if (msg.filterStory) {
                entries = entries.filter(e => (e.storyId ?? "").toLowerCase().includes(msg.filterStory.toLowerCase()));
            }
            ...
            const content = msg.format === "json"
                ...
                : (0, AuditLog_1.renderAuditHtml)(entries);
...
<script>
  ...
  function send(cmd, days) { vscode.postMessage({ command: cmd, days: days }); }
    ...
    _renderPanelHtml(entries, sizeBytes) {
        const operations = [...new Set(entries.map(e => e.operation))].sort();
        const stories = [...new Set(entries.map(e => e.storyId).filter(Boolean))].sort();
        ...
        const sizeWarn = sizeBytes >= SIZE_WARN_BYTES
            ...
            : "";
        const opOptions = operations.map(op => `<option value="${escapeHtml(op)}">${escapeHtml(AuditLog_1.OPERATION_LABELS[op] ?? op)}</option>`).join("");
        const storyOptions = stories.map(s => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join("");
        ...
        const rows = entries.slice().reverse().map(e => {
            ...
            const op = AuditLog_1.OPERATION_LABELS[e.operation] ?? e.operation;
...
//# sourceMappingURL=AuditTrailPanel.js.map