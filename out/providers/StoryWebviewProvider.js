...
class StoryWebviewProvider {
    ...
    _csp() {
        return (0, shared_1.cspMeta)(this._view.webview);
    }
    ...
    _getWebviewHtml(branch, storyId, progress, behindCount, coverageBlockedEnv, repoOverride, signoffPassed, localChanges, timelines, deletionAckPending = null, staleAgeDays = null, pendingActionsCount = 0, behindOriginCount = 0) {
        ...
        if (onFeatureBranch) {
            if (!devPublished) {
                ...
                const validateBtn = `<button class="btn ${isValidated ? "btn-secondary" : "btn-primary"}" onclick="send('validate', '${jsStr(nextEnv.name)}')">&#x2714; ${isValidated ? "Re-run Check" : "Check for Errors"} &mdash; ${escapeHtml(nextEnv.label)}</button>`;
        ...
        const envRows = environments.map((envCfg, idx) => {
            ...
            if (isFuture) {
                ...
                if (state === "published") {
                    ...
                    icon = "🔄";
                    label = "Checks passed — awaiting approval";
                }
                else if (state === "branch-created") {
                    icon = "🧪";
                    label = "Ready to check — run \"Check for Errors\" first";
                }
                if (isDone) {
                    icon = "✓";
                }
                ...
                if (isPublishStage && state === "published" && localChanges) {
                    ...
                    const fileRow = (f, badge) => `<li><span class="file-path" onclick="viewWorkingDiff('${jsStr(f)}')" title="View diff">${escapeHtml(f)}</span><span class="story-badge">${badge}</span></li>`;
                    const fileList = [
                ...
                if (isPublishStage) {
                    ...
                    if (isLead) {
                        stageLinks += ` <a href="#" title="Promote a story to ${escapeHtml(envCfg.label)} (pick from a list — opens a PR)" onclick="send('promote', '${jsStr(envCfg.name)}')">⬆</a>`;
                    }
                    if (isLead) {
