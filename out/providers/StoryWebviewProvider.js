    ...
    _getWebviewHtml(branch, storyId, progress, behindCount, coverageBlockedEnv, repoOverride, signoffPassed, localChanges, timelines, deletionAckPending = null, staleAgeDays = null, pendingActionsCount = 0) {
        ...
        if (onFeatureBranch) {
            if (!devPublished) {
                ...
                const validateBtn = `<button class="btn ${isValidated ? "btn-secondary" : "btn-primary"}" onclick="send('validate', '${jsStr(nextEnv.name)}')">&#x2714; ${isValidated ? "Re-validate" : "Validate Only"} &mdash; ${escapeHtml(nextEnv.label)}</button>`;
