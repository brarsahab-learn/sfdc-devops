...
exports.DiffViewerPanel = void 0;
...
class DiffViewerPanel {
    static createOrShow(gitHelper, fromRef, toRef) {
        ...
        const panel = vscode.window.createWebviewPanel(DiffViewerPanel.viewType, "Compare Branches", column, { enableScripts: true, retainContextWhenHidden: true });
        DiffViewerPanel._current = new DiffViewerPanel(panel, gitHelper, fromRef, toRef);
    }
    static refreshIfOpen(gitHelper) {
        DiffViewerPanel._current?._initialize(gitHelper);
    }
    constructor(panel, gitHelper, fromRef, toRef) {
        this._disposables = [];
        this._panel = panel;
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            if (msg.command === "loadDiff" && msg.fromRef && msg.toRef) {
                ...
                const warning = files[0]?._warning;
                ...
                await gitHelper.fetchOriginQuiet();
                const results = await Promise.all(msg.pairs.map(async (p) => ({
                    from: p.from,
                    to: p.to,
                ...
                const { before, after } = (0, DiffContentProvider_1.buildDiffUris)(msg.filePath, msg.fromRef, msg.toRef);
    ...
    async _initialize(gitHelper, fromRef, toRef) {
        ...
        const allEnvs = (0, config_1.getEnvironments)();
        const refs = [];
        if (currentBranch) {
        ...
        const seen = new Set();
        const uniqueRefs = refs.filter(r => { if (seen.has(r.ref)) {
            return false;
    ...
    _buildHtml(refs, defaultFrom, defaultTo, pipelinePairs = []) {
...
exports.DiffViewerPanel = DiffViewerPanel;
DiffViewerPanel.viewType = "sfDevopsDiffViewer";
//# sourceMappingURL=DiffViewerPanel.js.map