...
class StoryPipelinePanel {
    static createOrShow(gitHelper) {
        ...
        const panel = vscode.window.createWebviewPanel("sfDevopsPipeline", "Salesforce-DevOps — Story Pipeline", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
        StoryPipelinePanel._current = new StoryPipelinePanel(panel, gitHelper);
    }
    static refreshIfOpen() {
        StoryPipelinePanel._current?._refresh();
    }
    constructor(_panel, _gitHelper) {
        this._panel = _panel;
    ...
    async _buildCards() {
        ...
        const devEnv = (0, config_1.getPublishEnvironment)();
        ...
        const staleDay = (0, config_1.getStaleStoryThresholdDays)();
        const now = Date.now();
        ...
        const deployedInEnv = new Map();
        for (const env of allEnvs) {
            const ids = new Set();
            const log = await this._gitHelper.recentCommitsOnBranch(env.branch, 300);
            for (const c of log) {
                const id = (0, DeploymentPlanner_1.storyIdFromMessage)(c.message, pattern);
                if (id) {
                    ids.add(id);
                }
            }
        ...
        for (const branch of remoteBranches) {
            const storyId = (0, config_1.extractStoryId)(branch) || branch;
    ...
    _loadingHtml() {
        return (0, shared_1.loadingHtml)("Loading pipeline…");
    }
    _renderHtml(cards) {
        const envs = (0, config_1.getPromotableEnvironments)();
        const devEnv = (0, config_1.getPublishEnvironment)();
...
exports.StoryPipelinePanel = StoryPipelinePanel;
 ...
 */
function escapeForOnclickArg(s) {
    return escapeHtml(s.replace(/\\/g, "\\\\").replace(/'/g, "\\'"));
}
//# sourceMappingURL=StoryPipelinePanel.js.map