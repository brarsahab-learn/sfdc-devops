...
exports.CoverageWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const coverageCheck_1 = require("../commands/coverageCheck");
const config_1 = require("../config");
class CoverageWebviewProvider {
    constructor(_extensionUri, _gitHelper, _storyProvider) {
    ...
    async _runCheck(testNames) {
        ...
        if (!sourceOrgAlias) {
            vscode.window.showWarningMessage(`No org alias configured for the ${sourceOrgLabel} stage — set sfDevops.devOrgAlias (or that environment's orgAlias) before running the coverage check.`);
            return;
        }
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Running Apex tests in the ${sourceOrgLabel} org...`, cancellable: false }, async () => {
    ...
    _html(branch, storyId, apex, passed, stale = false) {
        ...
        if (!onFeature) {
            ...
            const resultBlock = this._lastResult && this._lastResult.ran && !this._lastResult.error
                ? `<div class="divider"></div><b>Last run</b>` +
                    this._lastResult.perClass.map(c => `<div class="row"><span>${c.name}</span><span style="color:${c.pass ? "var(--vscode-charts-green)" : "var(--vscode-errorForeground)"}">${c.percent}%</span></div>`).join("") +
...
exports.CoverageWebviewProvider = CoverageWebviewProvider;
CoverageWebviewProvider.viewType = "sfDevopsCoverageView";
//# sourceMappingURL=CoverageWebviewProvider.js.map