...
function metaSiblingPath(filePath) {
    if (filePath.endsWith("-meta.xml")) {
        return filePath.slice(0, -"-meta.xml".length) || null;
    }
    return `${filePath}-meta.xml`;
}
class DeploymentDashboardPanel {
    /** Opens (or rebinds, if already open) the dashboard to exactly one environment — see `_boundEnv`. */
    static createOrShow(gitHelper, context, env) {
        ...
        const panel = vscode.window.createWebviewPanel("sfDevopsDeploymentDashboard", "Salesforce-DevOps Deployments", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    constructor(panel, _gitHelper, _extContext, env) {
        ...
        this._validatedSelections = new Map();
        this._panel = panel;
        this._boundEnv = env;
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case "runAction":
                    ...
                    break;
                case "viewFileDiff":
                    await this._viewFileDiff(msg);
                    break;
                case "viewPendingFileDiff":
                    await this._viewPendingFileDiff(msg);
                    break;
                case "rollback":
                    await this._handleRollback(msg);
                    break;
    ...
    async refresh() {
        ...
        try {
            ...
            if (token !== this._refreshToken) {
                return;
            } // a newer refresh has since started — this one's result is stale
            ...
            const envs = [(0, config_1.getPublishEnvironment)(), ...(0, config_1.getPromotableEnvironments)()];
            ...
            if (token !== this._refreshToken) {
                return;
            } // stale by the time the view model finished building
    ...
    async _buildViewModel(env, nextEnv, prevEnv) {
        ...
        // this misses.
        const baselineBranch = prevEnv?.branch ?? (0, config_1.getBaseBranch)();
        ...
        if (baseline && currentSha && baseline !== currentSha) {
            const commits = await this._gitHelper.commitLogBetweenRaw(baseline, `origin/${env.branch}`);
            ...
            for (const c of commits) {
                ...
                for (const f of filesForCommit) {
                    if (!(f.path in fileDates)) {
                        fileDates[f.path] = c.date;
                    }
                }
            }
        ...
        if (nextEnv) {
            try {
                diffVsNext = await this._gitHelper.diffNameStatusBetween(env.branch, nextEnv.branch, sourceRoot);
            }
            catch {
                diffVsNext = null;
            }
        }
        const { xml: packageXml, unmapped } = (0, AuditLog_1.buildPackageXml)(allFiles);
        ...
        const pendingApexClasses = (0, DeploymentPlanner_1.apexClassNamesIn)(allFiles);
        const { apexTestMap, apexTestFilePaths } = pendingApexClasses.length > 0
            ? (0, DeploymentPlanner_1.buildApexTestMap)(await this._gitHelper.listFilesAtRef(env.branch, sourceRoot), pendingApexClasses)
        ...
        return {
            ...
            canDeploy: (0, config_1.canPromote)(this._userRole, env),
    ...
    async _executeStep(env, mode, selection, files, summary, testLevel, tests, onProgress) {
        ...
        if (result.success && mode === "deploy") {
            ...
            if (sha) {
                await this._gitHelper.recordDeployed(env.name, sha, { numberComponentsDeployed: result.numberComponentsDeployed });
            }
        }
    ...
    async _runAction(msg) {
        ...
        try {
            ...
            const requestedMode = msg.actionMode === "deploy" ? "deploy" : "validate";
            const selection = { mode: msg.selectionMode, storyIds: msg.storyIds, files: msg.files };
            ...
            let { files, summary } = (0, DeploymentPlanner_1.resolveSelection)(selection, model.groups, model.allFiles);
            ...
            if (selection.mode === "files") {
                ...
                for (const f of files) {
                    ...
                    if (sibling && !present.has(sibling)) {
                        ...
                        if (siblingFile) {
                            present.add(sibling);
                            extraMeta.push(siblingFile);
                        }
                    }
                }
                if (extraMeta.length > 0) {
                    files = [...files, ...extraMeta];
                }
            }
            ...
            const testMode = msg.testMode === "all" ? "all" : "auto";
            const { testLevel, tests } = (0, DeploymentPlanner_1.resolveEffectiveTestLevel)(env.deployTestLevel, testMode, (0, DeploymentPlanner_1.apexClassNamesIn)(files), model.apexTestMap, env.isProd, env.coverageGate ?? true);
            // TestSuite files reference test classes by name.  Salesforce validates those class
            // names exist in the org even for NoTestRun — on a first deployment the class hasn't
            // been deployed yet, causing "No classes found for <Class>".  Safe to skip when
            // NoTestRun because the suite is never executed anyway.
            ...
            if (testLevel === "RunSpecifiedTests" && selection.mode !== "all" && tests?.length) {
                ...
                for (const testName of tests) {
                    for (const p of model.apexTestFilePaths[testName] ?? []) {
                        if (!present.has(p)) {
                            present.add(p);
                            extra.push({ path: p, change: "modified" });
                        }
                    }
                }
                if (extra.length > 0) {
            ...
            if (selection.mode !== "all") {
                ...
                if (deletedFiles.length > 0) {
                    ...
                    vscode.window.showWarningMessage(`${deletedFiles.length} deleted file(s) can't be included in this ${requestedMode === "deploy" ? "deploy" : "validate"} yet ` +
            ...
            if (requestedMode === "deploy" && env.coverageGate) {
                ...
                if (failing.length > 0) {
                    const { threshold } = (0, coverageCheck_1.coverageSettings)();
                    const names = failing.map(f => f.storyId).join(", ");
                    const choice = await vscode.window.showWarningMessage(`${names} ${failing.length === 1 ? "has" : "have"} Apex classes without a passing Code Coverage check (≥ ${threshold}%) yet — ` +
                        `${env.label} requires it (Admin panel: Coverage Gate). Run the Code Coverage check first.`, "Open Coverage Panel");
            ...
            if (requestedMode === "deploy") {
                ...
                const demoNote = env.isProd && msg.deployToDemo && (0, config_1.getDemoOrgAlias)()
                    ? `\n\nAlso deploying the SAME selection to Demo (${(0, config_1.getDemoOrgAlias)()}) at the same time, in parallel.`
                    : "";
                const confirm = await vscode.window.showWarningMessage(`${msg.selectionMode === "all" ? "Deploy ALL pending changes" : "Deploy the selected changes"} to ${env.label} (${env.orgAlias})?\n\nThis runs a real Salesforce deployment.${demoNote}`, { modal: true }, "Yes, deploy");
                if (!confirm) {
                    return;
                }
            }
            ...
            if (await this._gitHelper.hasUncommittedChanges()) {
                stashLabel = await (0, GitHelper_1.warnUncommittedChanges)(this._gitHelper, "Commit or stash your local changes before deploying — this checks out a fresh copy of the environment branch from origin, which would collide with them. Note this deploy would never have included them anyway: it always deploys origin's pushed content, never local edits.", { offerStash: true });
                if (!stashLabel) {
                    return;
                }
            }
            ...
            const cleanupPrompt = { value: null };
            ...
            const broadcastProgress = (activeStep, message, done, total, completedSteps) => {
            ...
            const buildOnProgress = (vsProgress, completedStepsRef, currentStepRef) => (status) => {
                ...
                if (/conflict/i.test(status)) {
                    ...
                    if (!completedStepsRef.list.includes('conflicts')) {
                        completedStepsRef.list.push('conflicts');
                    }
                    step = 'submit';
                }
                else if (/component/i.test(status)) {
                    if (!completedStepsRef.list.includes('submit')) {
                        completedStepsRef.list.push('submit');
                    }
                    step = 'components';
                }
                else if (/test/i.test(status)) {
                    if (!completedStepsRef.list.includes('components')) {
                        completedStepsRef.list.push('components');
                    }
                    step = 'tests';
                }
            ...
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `${requestedMode === "deploy" ? "Deploying" : "Validating"} against ${env.label}...`, cancellable: false }, async (progress) => {
                try {
                    const completedSteps = { list: [] };
                    const currentStep = { value: 'checkout' };
                    ...
                    const runDemoInParallel = requestedMode === "deploy" && env.isProd && Boolean(msg.deployToDemo) && Boolean((0, config_1.getDemoOrgAlias)());
                    ...
                    if (demoResult) {
                        if (demoResult.success) {
                            vscode.window.showInformationMessage(`✅ Also deployed to Demo (${(0, config_1.getDemoOrgAlias)()}) in parallel — ${summary}.`);
                        }
                        else {
                    ...
                    if (requestedMode === "validate" && first.success && autoDeployRequested) {
                        (0, Log_1.log)(`Validate passed — auto-deploying to ${env.label} (auto-deploy enabled)…`);
                        ...
                        if (second.success) {
                            this._validatedSelections.delete(env.name);
                        }
                    ...
                    if (stashLabel) {
                        ...
                        if (restore.status === "conflict") {
                            vscode.window.showWarningMessage(`Your stashed changes are safe but conflicted while restoring — resolve the conflict markers now showing in your files (Source Control view), then run "git stash drop" to finish (stash: ${restore.ref}).`);
                        }
                    }
                    await this.refresh();
                }
     ...
     * a no-op there.
     */
    async _promptCleanupPromotionBranches(env, storyIds) {
        if (env.name === (0, config_1.getPublishEnvironment)().name || storyIds.length === 0) {
            return;
        }
        const branches = storyIds.map(id => ({ id, branch: this._gitHelper.promoBranchName(id, env.name, "promote") }));
        const existing = [];
        for (const b of branches) {
            if (await this._gitHelper.remoteBranchExists(b.branch)) {
                existing.push(b);
            }
        }
        ...
        const choice = await vscode.window.showInformationMessage(`${env.label} is deployed — delete the promotion branch${existing.length > 1 ? "es" : ""} now that ${existing.length > 1 ? "they're" : "it's"} done?\n\n${list}`, "Yes, delete", "No");
    ...
    async _handleRollback(msg) {
        ...
        const confirm = await vscode.window.showWarningMessage(`Roll back ${env.label} (${env.orgAlias}) to the state at commit ${lastDeploy.sha.slice(0, 8)}?\n\n` +
            `This redeploys the ENTIRE source folder at that snapshot to the org. Only use this to undo a bad promotion.`, { modal: true }, "Yes, roll back");
        if (!confirm) {
            return;
        }
        const lockKey = `dashboard:${env.name}`;
        ...
        let stashLabel = null;
        if (await this._gitHelper.hasUncommittedChanges()) {
            stashLabel = await (0, GitHelper_1.warnUncommittedChanges)(this._gitHelper, "Commit or stash your local changes before rolling back — this checks out a specific commit SHA.", { offerStash: true });
            if (!stashLabel) {
                this._gitHelper.endOperation(lockKey);
                return;
            }
        ...
        try {
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Rolling back ${env.label} to ${lastDeploy.sha.slice(0, 8)}…`, cancellable: false }, async (progress) => {
                ...
                const rollbackCompleted = { list: ['checkout'] };
                const rollbackCurrent = { value: 'components' };
                const rollbackOnProg = (status) => {
                ...
                const result = await this._executeStep(env, "deploy", { mode: "all", storyIds: [], files: [] }, [], `Rollback ${env.label} to ${lastDeploy.sha.slice(0, 8)}`, "NoTestRun", undefined, rollbackOnProg);
                if (result.success) {
            ...
            this._gitHelper.endOperation(lockKey);
            if (stashLabel) {
                await this._gitHelper.restoreStash(stashLabel).catch(() => { });
            }
            await this.refresh();
        }
    }
    /** Diff between two environment branches — used by the "diff vs next env" preview list. */
    async _viewFileDiff(msg) {
        if (!msg.path || msg.path.includes('..') || msg.path.startsWith('/')) {
            return;
        }
    ...
    async _viewPendingFileDiff(msg) {
        if (!msg.path || msg.path.includes('..') || msg.path.startsWith('/')) {
            return;
        }
        const publishEnv = (0, config_1.getPublishEnvironment)();
        ...
        const isDev = env.name === publishEnv.name;
        const promIdx = promotable.findIndex(e => e.name === env.name);
        const prevLabel = isDev ? (0, config_1.getBaseBranch)() : (promIdx > 0 ? promotable[promIdx - 1].label : publishEnv.label);
        const prevBranch = isDev ? (0, config_1.getBaseBranch)() : (promIdx > 0 ? promotable[promIdx - 1].branch : publishEnv.branch);
        const lastDeploy = await this._gitHelper.getDeployState(env.name);
    ...
    _loadingHtml() {
        return (0, shared_1.loadingHtml)("Loading deployment dashboard…");
    }
    _renderHtml(model) {
  ...
  document.querySelectorAll('.pane').forEach(function (p) {
    ...
    _renderEnvPane(m) {
        ...
        const outcomeHtml = outcome
            ? `<div class="outcome outcome-${outcome.kind}">${escapeHtml(outcome.message)}${outcome.nextEnv
        ...
        for (const f of m.allFiles) {
            const type = (0, AuditLog_1.metadataTypeForPath)(f.path);
            if (type) {
                if (!byType.has(type)) {
                    byType.set(type, []);
                }
...
<section class="env">
  ...
  <div class="deploy-row">
    ...
    ${env.isProd && (0, config_1.getDemoOrgAlias)() ? `<label class="auto-deploy-label" title="Runs a real deploy of this SAME selection to the Demo org (${escapeHtml((0, config_1.getDemoOrgAlias)())}) at the same time as Prod's own deploy — a genuinely parallel deploy, not one after the other. Demo's result is reported separately and never blocks or fails Prod's.">
      <input type="checkbox" id="deployToDemo-${env.name}">
      Also deploy to Demo (${escapeHtml((0, config_1.getDemoOrgAlias)())}) in parallel
    </label>` : ""}
...
exports.DeploymentDashboardPanel = DeploymentDashboardPanel;
...
function countTouchedGroups(groups, files) {
    const paths = new Set(files.map(f => f.path));
...
function touchedStoryIds(groups, files) {
    const paths = new Set(files.map(f => f.path));
...
//# sourceMappingURL=DeploymentDashboardPanel.js.map