...
class CoveragePanel {
    static createOrShow(gitHelper, storyProvider) {
        if (CoveragePanel._current) {
            CoveragePanel._current._panel.reveal(vscode.ViewColumn.Two);
            CoveragePanel._current._refresh();
            return;
        }
        const panel = vscode.window.createWebviewPanel("sfDevopsCoverage", "Code Coverage", vscode.ViewColumn.Two, { enableScripts: true, retainContextWhenHidden: true });
    ...
    static async openIfNeeded(gitHelper, storyProvider) {
        const branch = await gitHelper.currentBranch();
        const storyId = (0, config_1.isFeatureBranch)(branch) ? (0, config_1.extractStoryId)(branch) : "";
        if (!storyId) {
            return false;
        }
    ...
    constructor(_panel, _gitHelper, _storyProvider) {
        this._panel = _panel;
        this._gitHelper = _gitHelper;
        this._storyProvider = _storyProvider;
        this._disposables = [];
        this._lastTests = "";
        this._missingTests = [];
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (msg.command === "autoDetectTests") {
                this._lastTests = "";
                await this._refresh();
            }
    ...
    async _detectRelatedTests(branch, apex) {
        const sourceRoot = (0, config_1.getSourceRootFolder)();
        const files = await this._gitHelper.listFilesAtRef(branch, sourceRoot);
        const basenames = new Set(files.filter(f => f.endsWith(".cls")).map(f => f.split("/").pop().replace(/\.cls$/, "")));
        return (0, coverageCheck_1.findRelatedTestClasses)(apex, basenames);
    }
    async _runCheck(testNames) {
        const branch = await this._gitHelper.currentBranch();
        const storyId = (0, config_1.isFeatureBranch)(branch) ? (0, config_1.extractStoryId)(branch) : "";
        if (!storyId) {
        ...
        const { threshold, sourceOrgAlias, sourceOrgLabel } = (0, coverageCheck_1.coverageSettings)();
        const root = this._gitHelper.getWorkspaceRoot();
        if (!sourceOrgAlias) {
            vscode.window.showWarningMessage(`No org alias configured for ${sourceOrgLabel} — set sfDevops.devOrgAlias (or that environment's orgAlias) before running the coverage check.`);
            return;
        }
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Running Apex tests in ${sourceOrgLabel}…`, cancellable: false }, async () => {
            const result = await (0, coverageCheck_1.runApexCoverage)(root, apex, tests, threshold, sourceOrgAlias);
            ...
            if (result.error) {
                ...
                vscode.window.showInformationMessage(`✅ Coverage passed (≥ ${threshold}%). ${storyId} can now be promoted to ${gateEnv?.label ?? "the gated environment"}.`);
                ...
                vscode.window.showWarningMessage(result.testsFailed > 0
    ...
    _loadingHtml() {
        return (0, shared_1.loadingHtml)("Loading coverage…");
    }
    _renderHtml(branch, storyId, apex, passed, stale) {
        ...
        if (!onFeature) {
            ...
            const { threshold, sourceOrgLabel, sourceOrgAlias } = (0, coverageCheck_1.coverageSettings)();
            ...
            const classRows = apex.map(c => `<tr><td class="cls-name">${escapeHtml(c)}</td><td>${this._lastResult?.perClass.find(r => r.name === c)
                ? (() => {
                    const r = this._lastResult.perClass.find(rr => rr.name === c);
                    return `<span class="${r.pass ? "pct-ok" : "pct-fail"}">${r.percent}%</span>`;
                })()
                : "<span class='pct-na'>not run</span>"}</td></tr>`).join("");
