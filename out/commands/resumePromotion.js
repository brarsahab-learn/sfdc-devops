...
exports.resumePromotion = resumePromotion;
...
async function resumePromotion(bbClient, gitHelper, storyProvider) {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                if (await gitHelper.hasUncommittedChanges()) {
                    stashLabel = await (0, GitHelper_1.warnUncommittedChanges)(gitHelper, "Commit or stash your local changes before resuming — the validate step that follows checks out a fresh copy from origin, which would collide with them.", { offerStash: true });
                    if (!stashLabel) {
                        return;
                    }
                }
...
async function cancelPromotion(gitHelper, storyProvider) {
    const op = await gitHelper.getPendingOperation();
    if (!op) {
    ...
    const confirm = await vscode.window.showWarningMessage(`Cancel the ${label} for ${op.storyId}? Your resolved changes on this branch will be discarded.`, { modal: true }, "Yes, cancel");
    if (!confirm) {
        return;
    }
...
//# sourceMappingURL=resumePromotion.js.map