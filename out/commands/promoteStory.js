...
exports.reviewStoryDiff = reviewStoryDiff;
exports.promoteStory = promoteStory;
exports.runPromotion = runPromotion;
exports.finalizeAndFinish = finalizeAndFinish;
exports.openPromotionPR = openPromotionPR;
exports.reportOperationConflict = reportOperationConflict;
const vscode = __importStar(require("vscode"));
const GitHelper_1 = require("../GitHelper");
const coverageCheck_1 = require("./coverageCheck");
 ...
 */
async function reviewStoryDiff(gitHelper, featureBranch, targetBranch, files) {
    if (files.length === 0) {
        return;
    }
    const icon = (change) => change === "added" ? "$(diff-added)" : change === "deleted" ? "$(diff-removed)" : "$(diff-modified)";
    while (true) {
        const pick = await vscode.window.showQuickPick(files.map(f => ({ label: `${icon(f.change)} ${f.path}`, description: f.change, file: f })), {
        ...
        const { before, after } = (0, DiffContentProvider_1.buildDiffUris)(pick.file.path, targetBranch, featureBranch);
        await vscode.commands.executeCommand("vscode.diff", before, after, `${pick.file.path} (${targetBranch} ↔ ${featureBranch})`);
    }
}
/** Metadata changed on the story's feature branch vs base — used for the audit trail. */
async function storyChangedFiles(gitHelper, storyId) {
    try {
        return await gitHelper.diffNameStatusBetween((0, config_1.getBaseBranch)(), (0, config_1.featureBranchName)(storyId));
    }
    catch (err) {
        (0, Log_1.log)(`[warn] storyChangedFiles: could not diff ${(0, config_1.featureBranchName)(storyId)} — audit will have empty file list. (${err})`);
        return [];
    }
}
/**
 ...
 */
async function promoteStory(bbClient, gitHelper, targetEnv, mode, storyProvider) {
 ...
 */
async function runPromotionValidate(gitHelper, storyId, targetEnv, envCfg, progress) {
    ...
    let files = await gitHelper.diffNameStatusBetween(targetBranch, promotionBranch, (0, config_1.getSourceRootFolder)());
    if (files.length === 0) {
    ...
    if (deletedFiles.length > 0) {
        ...
        vscode.window.showWarningMessage(`${storyId}: ${deletedFiles.length} deleted file(s) can't be included in this validate/deploy yet ` +
    ...
    if (testLevel === "NoTestRun") {
        ...
        if (suiteFiles.length > 0) {
            ...
            (0, Log_1.log)(`Skipping ${suiteFiles.length} test suite file(s) — tests not required for this deployment.`);
        }
    }
    ...
    if (testLevel === "RunSpecifiedTests" && tests?.length) {
        ...
        if (extra.length > 0) {
            files = [...files, ...extra.map(p => ({ path: p, change: "modified" }))];
        }
    }
    progress?.report({ message: `Validating against ${targetEnv.toUpperCase()}...` });
    const result = await (0, DeploymentEngine_1.runDeploy)(gitHelper.getWorkspaceRoot(), (0, config_1.getSourceRootFolder)(), files.map(f => f.path), envCfg.orgAlias ?? "", testLevel, (0, config_1.getDeployTimeoutSeconds)(), "validate", tests, status => progress?.report({ message: status }));
    return { result, testLevel, tests };
}
async function runPromotion(bbClient, gitHelper, storyId, targetEnv, mode, storyProvider) {
    ...
    if (mode === "promote") {
        const promoBranchForPr = (0, config_1.promoBranchName)(storyId, targetEnv, "promote");
    ...
    try {
        ...
        if (mode === "promote" && envCfg?.coverageGate) {
            ...
            if (apex.length > 0 && !(await gitHelper.isCoveragePassed(storyId))) {
                ...
                const choice = await vscode.window.showWarningMessage(`${storyId} has Apex classes (${apex.slice(0, 4).join(", ")}${apex.length > 4 ? ", …" : ""}). ` +
                    `Run the Code Coverage check (≥ ${threshold}%) in the Code Coverage panel before promoting to ${envUpper}.`, "Open Coverage Panel");
        ...
        if (mode === "promote") {
            ...
            if (currentEnv?.signoffGate && !(await gitHelper.isSignoffPassed(storyId, currentEnv.name))) {
                vscode.window.showWarningMessage(`${currentEnv.label} sign-off hasn't been recorded for ${storyId} yet. Record it in the Current Story panel before promoting to ${envUpper}.`);
                return;
            }
        }
        ...
        if (await gitHelper.hasUncommittedChanges()) {
            stashLabel = await (0, GitHelper_1.warnUncommittedChanges)(gitHelper, `Commit or stash your local changes before ${mode === "validate" ? "validating" : "promoting"} — this checks out a fresh copy of the promotion branch from origin, which would collide with them.`, { offerStash: true });
            if (!stashLabel) {
                return;
            }
        }
        ...
        if (mode === "promote" && await gitHelper.promotionBranchExists(storyId, targetEnv)
            ...
            const confirm = await vscode.window.showWarningMessage(`Promote & Deploy ${storyId} to ${envUpper}?\n\nThe promotion branch already exists and has passed validation — this will open its PR (promotion → ${targetEnv}). ${envUpper} deploys once you approve & merge it.`, { modal: true }, "Yes, open PR");
            if (!confirm) {
                return;
            }
        ...
        const hasOwnPendingConflict = Boolean(pendingOp && pendingOp.kind === "promotion" && pendingOp.storyId === storyId && pendingOp.targetEnv === targetEnv);
        if (hasOwnPendingConflict) {
            vscode.window.showWarningMessage(`${storyId} → ${envUpper} has an unresolved conflict from an earlier attempt — resolve it and click Resume instead of Validate/Promote again.`);
            return;
        }
        const promoBranch = (0, config_1.promoBranchName)(storyId, targetEnv, "promote");
        ...
        if (deletedInPreview.length > 0) {
            ...
            if (!ack || ack.sha !== currentSha) {
                ...
                vscode.window.showWarningMessage(`❌ ${storyId} deletes ${deletedInPreview.length} metadata component(s) that cannot be deployed automatically yet: ${fileNames}${extra}.\n\nRemove them manually from the ${envUpper} org, then use "Acknowledge manual deletion" in the Current Story panel to unblock this promotion.`, { modal: true });
                return;
            }
        }
 ...
 */
async function finalizeAndFinish(bbClient, gitHelper, storyId, targetEnv, mode, storyProvider, progress) {
    const envUpper = targetEnv.toUpperCase();
    const envCfg = (0, config_1.findEnvironment)(targetEnv);
    if (!envCfg) {
    ...
    if (!valResult.success) {
        vscode.window.showErrorMessage(`❌ Validation against ${envUpper} failed: ${valResult.error ?? "see the audit trail"}. ` +
    ...
    if (mode === "validate") {
        ...
        vscode.window.showInformationMessage(`✅ Validated ${storyId} against ${envUpper} — ${valResult.numberComponentsDeployed ?? 0} component(s), no errors${testNote}. ` +
 ...
 */
async function openPromotionPR(bbClient, gitHelper, storyId, targetEnv, storyProvider, progress) {
    if (!(await gitHelper.isPromotionValidated(storyId, targetEnv))) {
        vscode.window.showErrorMessage(`Can't open a PR for ${storyId} → ${targetEnv.toUpperCase()} — validation hasn't passed for this promotion branch yet. Run Validate first.`);
        return;
    }
    const promotionBranch = (0, config_1.promoBranchName)(storyId, targetEnv, "promote");
    const targetBranch = (0, config_1.findEnvironment)(targetEnv)?.branch ?? targetEnv;
    progress?.report({ message: "③ Opening pull request page..." });
    await gitHelper.checkoutFeature(storyId);
    const changedFiles = await storyChangedFiles(gitHelper, storyId);
    const { xml: packageXml, unmapped: unmappedFiles, typeGroups } = (0, AuditLog_1.buildPackageXmlWithGroups)(changedFiles);
    // Auto-generate PR description from available metadata
    const ticketLink = (0, config_1.buildTicketUrl)(storyId);
    ...
    if (!prUrl) {
        ...
        vscode.window.showWarningMessage(`${promotionBranch} was pushed, but the repo could not be determined. ` +
    ...
    vscode.window.showInformationMessage(`✅ ${promotionBranch} pushed (already validated). Review & merge the PR (→ ${targetEnv}) in your browser — ` +
