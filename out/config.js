...
exports.checkProdEnvironmentConfig = checkProdEnvironmentConfig;
...
let _orgAliasMigrationPromise = null;
...
async function ensureOrgAliasMigration() {
    _ensureMigration();
    ...
    if (explicit) {
        try {
            ...
            if (!_ticketKeyPatternWarningShown) {
                ...
                vscode.window.showWarningMessage(`sfDevops: Invalid ticketKeyPattern "${explicit}" — ${e.message}. Using default pattern.`);
            }
        }
    }
 ...
 */
function checkProdEnvironmentConfig() {
    if (_prodEnvWarningShown) {
        return;
    }
    _prodEnvWarningShown = true;
    const hasProd = getEnvironments().some(e => e.isProd);
    if (!hasProd) {
        vscode.window.showWarningMessage('sfDevops: No production environment configured. Set "isProd": true on your production environment in settings.json to enable production safety guards.');
    }
}
/**
