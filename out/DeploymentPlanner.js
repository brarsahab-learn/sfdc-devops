...
function resolveEffectiveTestLevel(configuredLevel, testMode, apexClassesInSelection, apexTestMap, isProd = false) {
    ...
    return { testLevel: exports.VALID_TEST_LEVELS.includes(configuredLevel) ? configuredLevel : "RunLocalTests" };
}
...
function groupChangesByStory(commits, filesByHash) {
    ...
    for (let i = 0; i < groups.length; i++) {
        for (let j = i + 1; j < groups.length; j++) {
            ...
            if (shareFiles) {
                if (!a.sharedWith.includes(b.storyId)) {
                    a.sharedWith.push(b.storyId);
                }
                if (!b.sharedWith.includes(a.storyId)) {
                    b.sharedWith.push(a.storyId);
                }
            }
        }
    }
