...
exports.VALID_TEST_LEVELS = void 0;
exports.isLikelyTestClass = isLikelyTestClass;
exports.resolveEffectiveTestLevel = resolveEffectiveTestLevel;
exports.apexClassNamesIn = apexClassNamesIn;
exports.buildApexTestMap = buildApexTestMap;
exports.groupChangesByStory = groupChangesByStory;
exports.storyIdFromMessage = storyIdFromMessage;
exports.distinctStoryIdsFromCommits = distinctStoryIdsFromCommits;
...
function isLikelyTestClass(name) {
...
exports.VALID_TEST_LEVELS = ["NoTestRun", "RunSpecifiedTests", "RunLocalTests", "RunAllTestsInOrg"];
/**
 ...
 */
function resolveEffectiveTestLevel(configuredLevel, testMode, apexClassesInSelection, apexTestMap, isProd = false, coverageGate = true) {
    if (testMode === "all") {
    ...
    if (!isProd && !coverageGate) {
        return { testLevel: "NoTestRun" };
    }
    const detected = Array.from(new Set(apexClassesInSelection.map(name => apexTestMap[name]).filter((t) => Boolean(t))));
    if (detected.length > 0) {
        return { testLevel: "RunSpecifiedTests", tests: detected };
    }
    if (apexClassesInSelection.length > 0) {
    ...
    const validProdLevel = (configuredLevel !== "NoTestRun" && exports.VALID_TEST_LEVELS.includes(configuredLevel))
...
function apexClassNamesIn(files) {
    return Array.from(new Set(files.filter(f => f.path.endsWith(".cls") && (0, AuditLog_1.metadataTypeForPath)(f.path) === "ApexClass")
        .map(f => f.path.split("/").pop().replace(/\.cls$/, "")))).filter(name => !isLikelyTestClass(name));
}
/**
 ...
 */
function buildApexTestMap(allClsFiles, candidateClasses) {
    const basenameToPaths = new Map();
    for (const f of allClsFiles) {
        if (!f.endsWith(".cls") && !f.endsWith(".cls-meta.xml")) {
            continue;
        }
        const base = f.split("/").pop().replace(/\.cls(-meta\.xml)?$/, "");
        if (!basenameToPaths.has(base)) {
            basenameToPaths.set(base, []);
        }
        basenameToPaths.get(base).push(f);
    }
    const apexTestMap = {};
    const apexTestFilePaths = {};
    for (const name of candidateClasses) {
        ...
        if (match) {
            apexTestFilePaths[match] = basenameToPaths.get(match);
        }
    }
 ...
 */
function storyIdFromCommitMessage(message) {
    const idx = message.indexOf(": ");
    return idx > 0 ? message.slice(0, idx) : "other";
}
function groupChangesByStory(commits, filesByHash) {
    ...
    const fileToGroupIndices = new Map();
    groups.forEach((g, i) => {
    ...
    for (const indices of fileToGroupIndices.values()) {
        ...
        for (let a = 0; a < indices.length; a++) {
            for (let b = a + 1; b < indices.length; b++) {
                ...
                if (!ga.sharedWith.includes(gb.storyId)) {
                    ga.sharedWith.push(gb.storyId);
                }
                if (!gb.sharedWith.includes(ga.storyId)) {
                    gb.sharedWith.push(ga.storyId);
                }
            }
        }
    }
 ...
 */
function storyIdFromMessage(message, pattern) {
    ...
    if (colonIdx > 0) {
        return message.slice(0, colonIdx);
    }
 ...
 */
function distinctStoryIdsFromCommits(commits, pattern) {
    const byStory = new Map();
 ...
 */
function resolveSelection(selection, groups, allFiles) {
...
//# sourceMappingURL=DeploymentPlanner.js.map