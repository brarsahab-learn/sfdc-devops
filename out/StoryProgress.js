...
exports.getEnvState = getEnvState;
exports.getStoryTimelines = getStoryTimelines;
const config_1 = require("./config");
async function getStoryProgress(gitHelper, bbClient, storyId) {
    const progress = {};
 ...
 */
async function getEnvState(gitHelper, bbClient, storyId, env) {
    if (!storyId) {
        return "none";
    }
    try {
        ...
        const envCfg = (0, config_1.getEnvironments)().find(e => e.name === env);
        const envBranch = envCfg?.branch ?? env;
        const promotionBranch = (0, config_1.promoBranchName)(storyId, env, "promote");
        const validateBranch = (0, config_1.promoBranchName)(storyId, env, "validate");
 ...
 */
async function getStoryTimelines(gitHelper, storyId) {
    const timelines = {};
    if (!storyId) {
        return timelines;
    }
    const entries = await gitHelper.getAuditEntries();
    const latestEntry = (operation, env) => entries
    ...
    const latestPublishEntry = () => entries
        ...
        .sort((a, b) => b.timestamp.localeCompare(a.timestamp))[0];
    const publishEnv = (0, config_1.getPublishEnvironment)();
    for (const envCfg of (0, config_1.getEnvironments)()) {
        const env = envCfg.name;
        const envBranch = envCfg.branch;
        let deployment = { done: false };
        try {
            const storyCommitSha = await gitHelper.storyCommitShaOnBranch(envBranch, storyId);
            if (storyCommitSha) {
                const lastDeploy = await gitHelper.getDeployState(env);
        ...
        let validation = { done: false };
        try {
            const record = await gitHelper.getPromotionValidationRecord(storyId, env);
            if (record?.passed) {
                validation = { done: true, at: record.date };
            }
        }
        catch { /* leave as pending */ }
        const promoteEntry = latestEntry("promote", env);
        const promotion = promoteEntry ? { done: true, at: promoteEntry.timestamp } : { done: false };
        timelines[env] = { validation, promotion, deployment };
    }
    return timelines;
}
//# sourceMappingURL=StoryProgress.js.map