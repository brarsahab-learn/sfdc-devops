...
function getSeedInfo(workspaceRoot: string, config: DmConfig, sourceOrg?: string): { sobject: string; label: string; count: number; lastPulled: string | null }[] {
    const base = path.resolve(workspaceRoot, config.seedDir);
    const seedDir = sourceOrg ? path.join(base, safeOrgName(sourceOrg)) : base;
...
// For embedding JSON in a <script> tag — escapes </script> injection vectors.
...
// For embedding JSON in a double-quoted HTML onclick/onchange attribute.
// The HTML parser decodes &quot; → " before handing the value to the JS engine,
// so string literals round-trip correctly without breaking the attribute boundary.
function jsonAttr(data: unknown): string {
    return JSON.stringify(data)
        .replace(/&/g, "\\u0026")
        .replace(/</g, "\\u003c")
        .replace(/>/g, "\\u003e")
...
export class DataMigrationPanel {
    ...
    private _config: DmConfig = { objects: [], batchSize: 190, seedDir: ".git/sf-devops-dm/seed" };
    ...
    private _pullLog: Array<{ text: string; level: string }> = [];
    private _loadLog: Array<{ text: string; level: string }> = [];
    private _trackingLog: Array<{ text: string; level: string }> = [];
    ...
    private _currentOpSobject: string | undefined;
    private _currentOpType: "pull" | "load" | undefined;
            ...
            switch (msg.command) {
                ...
                case "autoSort": {
                    ...
                    await this._runExtBusy("Auto-sorting…", async () => {
                        ...
                        const seedDirForSort = sourceOrg ? path.join(seedBase, safeOrgName(sourceOrg)) : seedBase;
                        cfg = await autoSortByDependencies(msg.targetOrg, this._workspaceRoot, cfg, () => {}, seedDirForSort);
                        writeDmConfig(this._workspaceRoot, cfg);
                        this._config = cfg;
                    });
                    break;
                ...
                case "clearSeed": {
                    ...
                    const seedDir = msg.sourceOrg ? path.join(base, safeOrgName(msg.sourceOrg as string)) : base;
                ...
                case "viewSeedFile": {
                    ...
                    const seedDir = msg.sourceOrg ? path.join(base, safeOrgName(msg.sourceOrg as string)) : base;
                    const seedFile = path.join(seedDir, `${msg.sobject as string}.json`);
                    if (!fs.existsSync(seedFile)) { vscode.window.showWarningMessage(`No seed file found for ${msg.sobject}.`); break; }
                ...
                case "validateMigration": {
                    ...
                    await this._runExtBusy("Validating migration…", async () => {
                        ...
                        writeJobLog(vLogDir, this._trackingLog.map(l => `[${l.level.toUpperCase()}] ${l.text}`));
                        this._trackingCache = undefined;
                    });
                    break;
                }
    ...
    private _makeLogHandlers(logTarget: Array<{ text: string; level: string }>) {
    ...
    private async _startPull(sourceOrg: string, dryRun: boolean, sobject?: string): Promise<void> {
        ...
        if (!sobject) { this._activeTab = "pull"; }
        const ctrl = makeController();
        this._pullController = ctrl;
        const { onLog, onProgress } = this._makeLogHandlers(this._pullLog);
        this._refresh();
        try {
            await pullData(sourceOrg, this._workspaceRoot, this._config, onLog, onProgress, ctrl, { dryRun, dryRunSampleSize: 5, sourceOrg, objectFilter: sobject ? [sobject] : undefined });
            if (!dryRun) { writeJobLog(pullLogsDir(this._workspaceRoot), this._pullLog.map(l => `[${l.level}] ${l.text}`)); }
    ...
    private async _startLoad(targetOrg: string, dryRun: boolean, sobject?: string, retryFailedOnly = false): Promise<void> {
        ...
        this._loadLog = [];
        this._dryRunMode = dryRun;
        if (!sobject) { this._activeTab = "load"; }
        const ctrl = makeController();
            ...
            if (!dryRun) {
                try {
                    ...
                    if (report.totalDiscrepancies > 0) {
                        ...
                        await reconcileTracking(targetOrg, this._workspaceRoot, this._config, null, onLog);
                        this._trackingCache = undefined;
                    }
    ...
    private async _startPullAndLoad(sourceOrg: string, targetOrg: string, dryRun: boolean): Promise<void> {
        ...
        try {
            await pullData(sourceOrg, this._workspaceRoot, this._config, pullLog, pullProg, pullCtrl, { dryRun, dryRunSampleSize: 5, sourceOrg });
            if (!dryRun) { writeJobLog(pullLogsDir(this._workspaceRoot), this._pullLog.map(l => `[${l.level}] ${l.text}`)); }
            this._pullState = "done";
            ...
            if (pullCtrl.state !== "cancelled") {
                ...
                this._refresh();
                await loadData(targetOrg, this._workspaceRoot, this._config, loadLog, loadProg, loadCtrl, { dryRun, sourceOrg });
                if (!dryRun) {
                    try {
                        const report = await validateMigration(targetOrg, this._workspaceRoot, this._config, loadLog);
                        if (report.totalDiscrepancies > 0) {
                            loadLog(`Auto-reconciling ${report.totalDiscrepancies} object(s) with discrepancies…`, "info");
                            await reconcileTracking(targetOrg, this._workspaceRoot, this._config, null, loadLog);
                            this._trackingCache = undefined;
                        }
    ...
    private _renderHtml(vm: Awaited<ReturnType<DataMigrationPanel["_buildViewModel"]>>): string {
        const { config, sourceOrg, targetOrg, role, envs, tracking, trackingOrg, trackedOrgs, hasLog, seedInfo,
                availableOrgs, pullState, loadState, activeTab, dryRun, pullLog, loadLog, trackingLog, busy, busyLabel, lastError,
                currentOpSobject, currentOpType } = vm;
                ...
                <tr id="row-${esc(obj.id)}" class="${active ? "" : "inactive-row"}${isOpRunning ? " row-op-running" : ""}">
                    ...
                    <td class="row-actions">
                        ${isOpRunning
                            ...
                            : `<button class="btn btn-sm" title="Pull this object from source org" ${dis} onclick="startOp(this,'pullObject',{sobject:${jsonAttr(obj.sobject)},sourceOrg:${jsonAttr(sourceOrg)},dryRun:false})">↓ Pull</button>
                        <button class="btn btn-sm" title="Load this object to target org" ${dis} onclick="startOp(this,'loadObject',{sobject:${jsonAttr(obj.sobject)},targetOrg:${jsonAttr(targetOrg)},dryRun:false})">↑ Load</button>
        ...
        const renderLoadTab = () => {
            ...
            <div class="run-idle-card">
                ...
                <div style="display:flex;gap:10px;flex-wrap:wrap">
                    ...
                    ${recentLogs.length > 0 ? `<button class="btn" onclick="send('viewLoadLog',{targetOrg:document.getElementById('loadTargetOrg')?.value||${jsonAttr(targetOrg)}})" style="margin-left:auto">📄 Last Load Log</button>` : ""}
                </div>
                ...
                rows += `<tr class="${isObjRunning ? "row-op-running" : ""}">
                    ...
                    <td class="row-actions">
                        ${isObjRunning
                            ...
                            : `<button class="btn btn-sm" title="Re-pull this object from source org" ${dis} onclick="startOp(this,'pullObject',{sobject:${jsonAttr(obj)},sourceOrg:${jsonAttr(sourceOrg)},dryRun:false})">↓ Re-Pull</button>
        ...
        const renderExtIdsTab = () => {
            ...
            for (const obj of activeObjs) {
                ...
                rows += `<tr>
                    ...
                    <td>${status}</td>
                    <td class="row-actions">
                        <button class="btn btn-sm" ${dis} onclick="send('checkExtId',{sobject:${jsonAttr(obj.sobject)},targetOrg:${jsonAttr(targetOrg)}})">Re-check</button>
                    </td>
                </tr>`;
            }
            ...
            <div class="toolbar" style="margin-top:16px">
                <button class="btn btn-primary" ${dis} onclick="send('checkAllExtIds',{targetOrg:${jsonAttr(targetOrg)}})">Re-check All</button>
