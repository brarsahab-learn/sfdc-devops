...
async function resolveRecordTypesForTarget(
    ...
    try {
        ...
        const parsed = JSON.parse(stdout);
        const recs: any[] = parsed?.result?.records ?? [];
        return new Map(recs.map(r => [r.DeveloperName as string, r.Id as string]));
    } catch {
        return new Map();
    }
}
...
export async function pullData(
    ...
    const objPullStatus = new Map<string, "pending" | "running" | "done" | "skipped">(
        objects.map(o => [o.sobject, "pending"])
    );
    let objectsDone = 0;

    // Pull one object — extracted so it can be called concurrently within a level.
    const pullOne = async (obj: DmObjectConfig): Promise<void> => {
        ...
        if (!pullSuccess) {
            ...
            onProgress({
                ...
                objectStatuses: objects.map(o => ({
                    sobject: o.sobject,
                    status: (objPullStatus.get(o.sobject) ?? "pending") as "done" | "running" | "pending" | "skipped",
                    created: 0, total: 1,
                })),
    ...
    const levelDeps = new Map<string, Set<string>>(
        objects.map(o => [o.sobject, new Set((o.dependsOn ?? []).filter(d => objectSet.has(d)))])
    );
    const launched = new Set<string>();
    const completed = new Set<string>();

    while (launched.size < objects.length) {
        if (stateOf(ctrl) === "cancelled") { break; }
        // Find objects whose dependencies are all already completed
        const ready = objects.filter(o =>
            !launched.has(o.sobject) &&
        ...
        for (const o of ready) { launched.add(o.sobject); }
        await Promise.all(ready.map(obj => pullOne(obj)));
        for (const o of ready) { completed.add(o.sobject); }
    }
...
function buildUpsertCsv(
    ...
    for (const rf of referenceFields) {
        if (rf.field === "RecordTypeId") { continue; }
        ...
        if (managedTypes.length > 1 && onWarn) {
            onWarn(
                `Polymorphic field '${rf.field}' on ${obj.sobject} references multiple migrated types: [${managedTypes.join(", ")}]. ` +
...
function readSeedRecords(seedDir: string, sobject: string, onWarn?: (msg: string) => void): Record<string, any>[] {
...
async function loadDataImpl(
    ...
    if (fs.existsSync(planPath)) {
        try {
            ...
            if (typeof plan.generatedAt === "string") {
                const ageDays = (Date.now() - new Date(plan.generatedAt).getTime()) / 86_400_000;
                if (ageDays > 7) {
                    onLog(
                        `⚠  plan.json was generated ${Math.round(ageDays)} days ago (${plan.generatedAt}). ` +
                        `If the object schema changed since then, relationship fields may be incorrect — consider re-pulling to regenerate it.`,
                        "warn",
                    );
                }
            }
    ...
    if (!dryRun) {
        for (const obj of objectList) {
            ...
            if (deps.includes(obj.sobject)) {
                emit(
                    `⚠  '${obj.sobject}' has a self-referential lookup (it depends on itself). ` +
                    ...
                    `(2) then run a second load using a SOQL that selects only Id + the lookup field to patch the references.`,
                    "warn",
                );
            }
        }
    ...
    if (!dryRun) {
        for (const obj of objectList) {
            ...
            if (seedRecordsForCheck.length === 0) { continue; }
            ...
            for (const rf of refFieldsForCheck) {
                ...
                if (isSystemField && rf.field in sampleRecord && sampleRecord[rf.field]) {
                    emit(
                        `⚠  Field '${rf.field}' on ${obj.sobject} references a system type (${rf.referenceTo.join("/")}) not in the migration set. ` +
                        `Its source org SF ID will be sent as-is to the target — remove it from the SOQL query or it will cause failures. ` +
                        `Exception: OwnerId defaults to the running user when omitted.`,
                        "warn",
                    );
                }
            }
        ...
        for (const o of missingParents) {
            try {
                ...
                const { stdout } = await execSf(
                    ...
                    { cwd: workspaceRoot, timeout: 300_000, maxBuffer: 200 * 1024 * 1024 },
                );
                const recs: any[] = JSON.parse(stdout)?.result?.records ?? [];
                const idMap = new Map<string, string>();
                for (const r of recs) {
    ...
    for (let objIdx = 0; objIdx < objectList.length; objIdx++) {
        ...
        if (objFailures.length > 0) {
            const failuresDir = path.join(workspaceRoot, ".git", "sf-devops-dm", "failures", safeOrgName(targetOrg));
            fs.mkdirSync(failuresDir, { recursive: true });
            const failurePath = path.join(failuresDir, `${safeOrgName(obj.sobject)}.json`);
...
async function loadBatch(
    ...
    const rtDevNames = [...new Set(
        records
            .map(r => r.RecordTypeId)
            .filter((v): v is string => typeof v === "string" && v.startsWith("__RecordType__"))
    ...
    const warnedPolymorphic = new Set<string>();
    const csv = buildUpsertCsv(records, obj, referenceFields, objByName, parentExtIdBySourceId, msg => {
        if (!warnedPolymorphic.has(msg)) { warnedPolymorphic.add(msg); emit(`⚠  ${msg}`, "warn"); }
    });
    if (!csv) { return; }
        ...
        if (planFields && planFields.length > 0) {
            ...
            for (const rf of planFields) {
                for (const ref of rf.referenceTo) {
                    if (sobjectNames.has(ref) && ref !== obj.sobject) { deps.add(ref); }
                }
            }
...
export async function rollbackData(
    ...
    for (const obj of objects) {
        ...
        const createdEntries = Object.entries(objTracking).filter(
            ([, e]) => e.status === "created" && e.id && e.wasNew !== false
        );
